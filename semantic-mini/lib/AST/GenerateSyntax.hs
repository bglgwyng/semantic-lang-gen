...
import AST.Unmarshal qualified as TS
...
makeHasFieldInstance ty elim =
  [d|
    instance HasField "ann" ($ty a) a where
...
symbolMatchingInstance allSymbols name named (DatatypeName str) = do
  ...
  [d|
    instance TS.SymbolMatching $(conT name) where
      matchedSymbols _ = $(lift tsSymbols)
      showFailure _ node =
        "expected "
          <> $(litE (stringL names))
          <> " but got "
          <> if nodeSymbol node == 65535
            then "ERROR"
            else
              genericIndex debugSymbolNames (nodeSymbol node)
                <> " ["
                <> show r1
                <> ", "
                <> show c1
                ...
                <> "]"
       where
        TSPoint r1 c1 = nodeStartPoint node
...
ctorForProductType constructorName children fields = ctorForTypes constructorName lists
 where
...
ctorForLeafType name annParameterName =
  ctorForTypes
    name
