...
import Text.Printf
import TreeSitter.Arith qualified as Arith (getNodeTypesPath, tree_sitter_arith)
import TreeSitter.Language qualified as TreeSitter.Language
...
emit language node_types_json_path = do
  decls <- T.pack . pprint . fmap adjust <$> astDeclarationsIO (Arith.tree_sitter_arith) node_types_json_path
...
main :: IO ()
main = do
  emit "Arith" =<< Arith.getNodeTypesPath
